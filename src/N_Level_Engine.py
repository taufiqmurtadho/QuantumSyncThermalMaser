...
#get phase distribution function
def phase_distribution(steadystate):
    D = len(steadystate)
    def S_function(phases):
        normalization_factor = (np.pi**(D-2))*(2**(D+1))
        phases.insert(0,0) #insert reference phase to the input
        res = 0
        for i in range(1,D):
            for j in range(1, D):
                if j != i:
                    res+=steadystate[i,j]*np.exp(1j*(phases[j-1]-phases[i-1]))
